...
void Hamiltonian::HTBCreate()
{
    ...
    for (l = 0; l < ncells_; l++)
    {
        ...
        // Intra Unit Cell Hopping [When Unit cell has more than one atom]
        for (int spin = 0; spin < 2; spin++){
            for(int orb1=0;orb1<n_orbs_;orb1++){
                for(int orb2=0;orb2<n_orbs_;orb2++){
                    if(IntraCell_Hopp(orb1,orb2)!=0.0){

                        a = Coordinates_.Nbasis(lx_pos,ly_pos,orb1) + ncells_*n_orbs_*spin;
                        b = Coordinates_.Nbasis(lx_pos,ly_pos,orb2) + ncells_*n_orbs_*spin;
                        assert(a != b);
                        if (a != b)
                        {
                            HTB_(b, a) = complex<double>(1.0 *IntraCell_Hopp(orb1,orb2) , 0.0);
        ...
        for (int spin=0; spin<2; spin++){
            for(int orb1=0;orb1<n_orbs_;orb1++){
                for(int orb2=0;orb2<n_orbs_;orb2++){
                    if(InterCell_px_Hopp(orb1,orb2)!=0.0){
                        a = Coordinates_.Nbasis(lx_pos,ly_pos,orb1) + ncells_*n_orbs_*spin;
                        b = Coordinates_.Nbasis(mx_pos,my_pos,orb2) + ncells_*n_orbs_*spin;
                        assert(a != b);
                        if (a != b)
                        {
                            HTB_(b, a) = complex<double>(1.0 *InterCell_px_Hopp(orb1,orb2), 0.0) * phasex;
                            HTB_(a, b) = conj(HTB_(b, a));
                        }
                    }
                }
        ...
        for (int spin = 0; spin < 2; spin++){
            for(int orb1=0;orb1<n_orbs_;orb1++){
                for(int orb2=0;orb2<n_orbs_;orb2++){
                    if(InterCell_py_Hopp(orb1,orb2)!=0.0){
                        ...
                        if (a != b)
                        {
                            HTB_(b, a) = complex<double>(1.0*InterCell_py_Hopp(orb1,orb2), 0.0) * phasey;
                            HTB_(a, b) = conj(HTB_(b, a));
                        }
                    }
                }
        ...
        // * +x-y direction Neighbor, without Twisted Boundary?

        m = Coordinates_.neigh(l, 7); //+x-y neighbour cell
        mx_pos = Coordinates_.indx_cellwise(m);
        my_pos = Coordinates_.indy_cellwise(m);
        ...
        for (int spin = 0; spin < 2; spin++){
            for(int orb1=0;orb1<n_orbs_;orb1++){
                for(int orb2=0;orb2<n_orbs_;orb2++){
                    if(InterCell_pxmy_Hopp(orb1,orb2)!=0.0){
                        ...
                        if (a != b)
                        {
                            HTB_(b, a) = complex<double>(1.0*InterCell_pxmy_Hopp(orb1,orb2), 0.0);
                            HTB_(a, b) = conj(HTB_(b, a));
                        }
                    }
                }
                ...
                for(int orb1=0;orb1<n_orbs_;orb1++){
                    for(int orb2=0;orb2<n_orbs_;orb2++){
                        if(IntraCell_Hopp(orb1,orb2)!=0.0){
                            ...
                            b = CoordinatesCluster_.Nbasis(lx_pos,ly_pos,orb2) + ncells_cluster*n_orbs_*spin;
                            assert(a != b);
                            if (a != b)
                            {
                                HTBCluster_(b, a) = complex<double>(1.0 *IntraCell_Hopp(orb1,orb2) , 0.0);
                ...
                for(int orb1=0;orb1<n_orbs_;orb1++){
                    for(int orb2=0;orb2<n_orbs_;orb2++){
                        if(InterCell_px_Hopp(orb1,orb2)!=0.0){
                            ...
                            if (a != b)
                            {
                                HTBCluster_(b, a) = complex<double>(1.0 *InterCell_px_Hopp(orb1,orb2), 0.0);
                                HTBCluster_(a, b) = conj(HTBCluster_(b, a));
                            }
                        }
                    }
                ...
                for(int orb1=0;orb1<n_orbs_;orb1++){
                    for(int orb2=0;orb2<n_orbs_;orb2++){
                        if(InterCell_py_Hopp(orb1,orb2)!=0.0){
                            ...
                            if (a != b)
                            {
                                HTBCluster_(b, a) = complex<double>(1.0 *InterCell_py_Hopp(orb1,orb2), 0.0);
                                HTBCluster_(a, b) = conj(HTBCluster_(b, a));
                            }
                        }
                    }
                ...
                for(int orb1=0;orb1<n_orbs_;orb1++){
                    for(int orb2=0;orb2<n_orbs_;orb2++){
                        if(InterCell_pxmy_Hopp(orb1,orb2)!=0.0){
                            ...
                            if (a != b)
                            {
                                HTBCluster_(b, a) = complex<double>(1.0 *InterCell_pxmy_Hopp(orb1,orb2), 0.0);
                                HTBCluster_(a, b) = conj(HTBCluster_(b, a));
                            }
                        }
                    }
...
{
    //Creating 3x3 Mat for hopping for intra and inter unit cell,
    //remember orbitals means different atoms here
    //Convention used
    //orb=0=d (Copper)
    //orb=1=px (Oxygen-1)
    //orb=2=py (Oxygen-2)

    IntraCell_Hopp.resize(3,3);
    IntraCell_Hopp(0,2)=-Parameters_.t_pd;
    IntraCell_Hopp(0,1)=-Parameters_.t_pd;
    IntraCell_Hopp(1,2)=-Parameters_.t_pp;

    InterCell_px_Hopp.resize(3,3);
    InterCell_px_Hopp(1,0)=Parameters_.t_pd; //Hopping from orb=1 to orb0 in +x direction: CONVENTION
    InterCell_px_Hopp(1,2)=Parameters_.t_pp;

    InterCell_py_Hopp.resize(3,3);
    InterCell_py_Hopp(2,0)=Parameters_.t_pd;
    InterCell_py_Hopp(2,1)=Parameters_.t_pp;

    InterCell_pxmy_Hopp.resize(3,3);
    InterCell_pxmy_Hopp(1,2)=-Parameters_.t_pp;
